...
class GradeRetrievalQuestion(BaseModel):
    ...
    score: str = Field(description="Whether the retrieved context is relevant to the question, 'yes' or 'no'")
...
class GradeRetrievalAnswer(BaseModel):
    ...
    score: str = Field(description="Whether the retrieved context is relevant to the answer, 'yes' or 'no'")
...
class OpenAIRelevanceGrader:
    ...
    def __init__(self, llm, target="retrieval-question"):
        ...
        if target == "retrieval-question":
            self.structured_llm_grader = llm.with_structured_output(GradeRetrievalQuestion)
        elif target == "retrieval-answer":
            self.structured_llm_grader = llm.with_structured_output(GradeRetrievalAnswer)
        else:
            raise ValueError(f"Invalid target: {target}")

        # 프롬프트
        target_variable = "user question" if target == "retrieval-question" else "answer"
